...
export const registerUser =
  ...
  // SELLER FILE UPLOAD
    if (role === "seller") {
      ...
      if (!licenseFile) { throw new ApiError(400,"License document required");}

      // Upload to cloudinary
      const uploadedFile = await new Promise((resolve, reject) => 
          {cloudinary.uploader.upload_stream({folder:"LeaseStay/licenses",resource_type:"auto",},
                (error, result) => {
  ...
  // SEND EMAIL
    ...
    await sendMail({
      to: user.email,
      subject:role === "seller"? "Welcome Seller to LeaseStay": "Welcome to LeaseStay",
      html: template,
    });

    return res.status(201).json(new ApiResponse(201,"Registration successful",{user,token,}));
...
export const getCurrentUser = asyncHandler(async (req, res) => {
    return res.status(200).json(new ApiResponse(200,"Current user fetched",req.user));
  });
